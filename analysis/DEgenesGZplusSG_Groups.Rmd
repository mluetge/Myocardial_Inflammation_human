...
saveRDS(seurat, paste0(basedir, "/data/humanHeartsPlusGraz_intPatients_merged",
...
GOcons <- lapply(clVec, function(cl){
  ...
  egoSS <- enrichGO(gene      = unique(clustDE_DatSub$ENS),
                ...
                qvalueCutoff  = 0.1)
  egoSS <- setReadable(egoSS, OrgDb = org.Hs.eg.db)
  egoSSres <- egoSS@result %>% filter(p.adjust < 0.1) %>% 
    mutate(subset=cl)
})
