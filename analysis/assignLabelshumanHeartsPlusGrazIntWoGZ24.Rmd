...
## high activation signature in GZ24 (very high CCL2) --> excluded
table(seurat$ID)
seurat <- subset(seurat, ID %in% c("GZ19", "SG30", "GZ24"), invert=T)
table(seurat$ID)
...
seuratSub <- subset(seurat, label=="Tcell")
seuratSub <- rerunSeurat3(seuratSub)
cd8 <- colnames(seuratSub)[which(seuratSub$RNA_snn_res.0.4 %in% c("1", "2", "3", "5", "7"))]
endo <- colnames(seuratSub)[which(seuratSub$RNA_snn_res.0.4 %in% c("4"))]
cd4 <- colnames(seuratSub)[-which(colnames(seuratSub) %in% c(cd8,endo))]
seurat$label2[which(colnames(seurat) %in% cd8)] <- "CD8T"
seurat$label2[which(colnames(seurat) %in% cd4)] <- "CD4T"
seurat$label2[which(colnames(seurat) %in% endo)] <- "Endothelial"
seurat$label[which(colnames(seurat) %in% endo)] <- "Endothelial"
...
colLab2 <- c(pal_futurama()(8), pal_uchicago()(6))[1:length(unique(seurat$label2))]
...
names(colLab2) <- unique(seurat$label2)
...
### label2
```{r vis label2}
DimPlot(seurat, reduction = "umap",  group.by = "label2", cols=colLab2)+
  theme_bw() +
  theme(axis.text = element_blank(), axis.ticks = element_blank(), 
        panel.grid.minor = element_blank()) +
  xlab("UMAP1") +
...
### per patient label2
```{r cnt tables per pat label2, fig.height=6, fig.width=8}

## celltype per patient counts
knitr::kable(table(seurat$label2, seurat$ID))

## celltype percentages per patient
datLab <- data.frame(table(seurat$label2, seurat$ID))
colnames(datLab) <- c("label", "ID", "cnt")
datPat <- data.frame(table(seurat$ID))
colnames(datPat) <- c("ID", "total")
datFrac <- datLab %>% left_join(., datPat, by="ID") %>% 
...
ordVec <- datFrac %>% dplyr::filter(label=="CD4T") %>% 
  arrange(., percentage)
ordBar <- c("CD4T","CD8T","infMacrophage","resMacrophage","Fibroblast","Perivascular",
            "SMC","Endothelial","EndoEC","LEC","Cardiomyocyte","Adipocytes",
            "NeuralCells")
datFrac <-datFrac %>% mutate(labelFac=factor(label, levels = ordBar))
ggbarplot(datFrac, x="ID", y="percentage",
...
patTot <- datFrac %>% select(ID, total) %>% distinct(.)
datCD48 <- datFrac %>% dplyr::filter(label %in% c("CD4T", "CD8T")) %>% 
  select(label, ID, cnt) %>% spread(., label, cnt) %>%
  mutate(CD4_to_CD8_ratio=CD4T/CD8T) %>% left_join(., patTot, by = "ID") %>% 
  mutate(CD4_fraction_of_total_cells=CD4T/total) %>%
  mutate(CD8_fraction_of_total_cells=CD8T/total)
knitr::kable(datCD48)
write.table(datCD48, file = paste0(basedir, "/data/CD4toCD8ratios.txt"),
            quote = F, row.names = F)
```
...
saveRDS(seurat, file = paste0(basedir, 
                              "/data/humanHeartsPlusGraz_intPatients_merged", 
                              "labeled_woGZ24_CD4CD8_seurat.rds"))
```
