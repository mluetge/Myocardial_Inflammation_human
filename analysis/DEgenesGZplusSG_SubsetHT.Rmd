...
myoGrp <- unique(seurat$ID[seurat$TcellGrp == "TcellHigh"])
myoGrp <- myoGrp[-which(is.na(myoGrp))]
myoLTGrp <- unique(seurat$ID[seurat$TcellGrp == "TcellLow"])
myoLTGrp <- myoLTGrp[-which(is.na(myoLTGrp))]
CtrlGrp <- unique(seurat$ID[seurat$TcellGrp == "Ctrl"])
CtrlGrp <- CtrlGrp[-which(is.na(CtrlGrp))]

seurat$cond2 <- seurat$TcellGrp
seurat$cond2[which(seurat$ID %in% myoGrp)] <- "TcellHigh"
seurat$cond2[which(seurat$ID %in% myoLTGrp)] <- "TcellLow"
...
write.table(GOconsDat, quote=F, row.names = T, col.names = T, sep= "\t",
            file = paste0(basedir,"/data/humanHeartsPlusGraz_intPatients_", 
                              "merged_SubsetHT_overallDEGO.txt"))
...
```{r vis sel genes violin Fibroblasts}
