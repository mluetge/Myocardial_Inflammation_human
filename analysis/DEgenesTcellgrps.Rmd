...
## all grps between 2500 and 5000 cells --> no need to downsample
...
GOcons <- lapply(grpVec, function(cl){
  ...
  egoSS <- enrichGO(gene      = unique(DEgenesSub$ENS),
                ...
                qvalueCutoff  = 0.1)
  egoSS <- setReadable(egoSS, OrgDb = org.Hs.eg.db)
  egoSSres <- egoSS@result %>% filter(qvalue < 0.1) %>% 
    mutate(subset=cl)
})
...
lapply(grpVec, function(cl){
  ...
  p <- cnetplot(egoSS, showCategory=100, node_label="all", 
